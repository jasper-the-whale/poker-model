...
fun List<Card>.isPair(): Boolean =
    this.groupBy { it.value }.entries.any { it.value.size == 2 }

fun List<Card>.isTriple(): Boolean =
    this.groupBy { it.value }.entries.any { it.value.size == 3 }

fun List<Card>.isQuadruple(): Boolean =
    this.groupBy { it.value }.entries.any { it.value.size == 4 }

fun List<Card>.isTwoPair(): Boolean =
    this.groupBy { it.value }.entries.size

fun List<Card>.isFullHouse(): Boolean =
    this.map { it.value }.distinct().size != this.size

fun List<Card>.isFlush(): Boolean =
    this.groupBy { it.suit }.entries.any { it.value.size >= 5 }

fun List<Card>.isStraight(): Boolean {
    val orderedList = this.sortedBy { it.value.valueNumber }.distinctBy { it.value.valueString }
    println(orderedList)
    val orderedPictureLists = (0..orderedList.size - 5).map {
        (0..4).fold("",
            { accumulator, index -> "$accumulator${orderedList[it + index].value.valueString}" }
        )
    }
